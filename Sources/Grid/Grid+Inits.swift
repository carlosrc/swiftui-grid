...
extension Grid {
    public init<Data, Item>(_ data: Data, onTap: @escaping (Any) -> (Void) = {_ in }, @ViewBuilder item: @escaping (Data.Element) -> Item) where Content == ForEach<Data, Data.Element.ID, Item>, Data : RandomAccessCollection, Item : View, Data.Element : Identifiable {
        self.items = data.map { GridItem(object: item($0), view: AnyView(item($0)), id: AnyHashable($0.id)) }
        self.onTapItem = onTap
    }

    public init<Data, ID, Item>(_ data: Data, id: KeyPath<Data.Element, ID>, onTap: @escaping (Any) -> (Void) = {_ in }, @ViewBuilder item: @escaping (Data.Element) -> Item) where Content == ForEach<Data, ID, Item>, Data : RandomAccessCollection, ID : Hashable, Item : View {
        self.items = data.map { GridItem(object: item($0), view: AnyView(item($0)), id: AnyHashable($0[keyPath: id]))
        }
        self.onTapItem = onTap
    }

    public init<Item>(_ data: Range<Int>, @ViewBuilder item: @escaping (Int) -> Item) where Content == ForEach<Range<Int>, Int, Item>, Item : View {
        self.items = data.map { GridItem(object: item($0), view: AnyView(item($0)), id: AnyHashable($0)) }
    }
}
